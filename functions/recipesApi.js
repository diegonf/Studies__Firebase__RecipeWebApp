...
app.post('/recipes', async (request, response) => {
  ...
  try {
    ...
    if(error instanceof Error){
      response.status(401).send(`Error Message not authorized: ${error.message}`);
    }
    return;
  }
  ...
  try {
    ...
    if(error instanceof Error){
      response.status(400).send(`Error Message adding recipe: ${error.message}`);
    }
    return;
  }
...
app.put('/recipes/:id', async (request, response) => {
  ...
  try {
    ...
    response.status(401).send(`Error Authorization: ${error.message}`);
    return;
  }
  ...
  try {
    ...
    response.status(400).send(`Error setting doc: ${error.message}`);
  }
